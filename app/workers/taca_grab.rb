  	...
  	taca_messages.each do |message|
  		if Trip.find_by_message_id(message.message_id).nil?
	  		...
	  		depart_times.each_with_index do |value, index|
	  			...
	  			depart_airport = Airport.find_by_faa(email.scan(/From:(.*?)Depart:/)[index].first.scan(/\((.*?)\)/).first.first).id
	  			arrival_airport = Airport.find_by_faa(email.scan(/To:(.*?)Arrive:/)[index].first.scan(/\((.*?)\)/).first.first).id
